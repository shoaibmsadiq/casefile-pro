...
import { Bell, BellOff, Paperclip, UploadCloud, FileWarning, FilePlus, Search, Edit, Trash2, X, Calendar, Briefcase, User, Users, MapPin, Tag, ChevronDown, LogOut, Loader2 } from 'lucide-react';
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            const sortedCases = casesData.sort((a, b) => {
                const dateA = a.hearingDates && a.hearingDates.length > 0 ? new Date(a.hearingDates[a.hearingDates.length - 1]) : 0;
                const dateB = b.hearingDates && b.hearingDates.length > 0 ? new Date(b.hearingDates[b.hearingDates.length - 1]) : 0;
                return dateB - dateA;
            });
        ...
        <>
            ...
            <main className="container mx-auto p-4 sm:p-6 lg:p-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-white p-6 rounded-xl shadow-lg col-span-1"><h3 className="font-bold text-slate-800 text-lg mb-4">Total Cases</h3><p className="text-5xl font-bold text-blue-600">{cases.length}</p></div>
                    <div className="bg-white p-6 rounded-xl shadow-lg col-span-1 md:col-span-2"><h3 className="font-bold text-slate-800 text-lg mb-4">Upcoming Hearings</h3>{upcomingHearings.length > 0 ? <ul className="space-y-2">{upcomingHearings.map((item, index) => <li key={`${item.id}-${index}`} className="flex justify-between items-center text-sm p-2 rounded-md bg-slate-50"><span className="font-semibold text-slate-700">{item.caseTitle}</span><span className="text-slate-500">{item.hearingDate.toLocaleDateString()}</span></li>)}</ul> : <p className="text-slate-500 text-center py-4">No upcoming hearings.</p>}</div>
                </div>
...
function CaseFormModal({ isOpen, onClose, onSave, caseData }) {
    ...
    useEffect(() => {
        if (caseData) setFormData({ ...caseData, hearingDates: caseData.hearingDates || [], tags: caseData.tags || [], attachments: caseData.attachments || [] });
        else setFormData({ clientName: '', caseTitle: '', courtName: '', caseNumber: '', hearingDates: [], opposingParty: '', fileLocation: '', tags: [], notes: '', attachments: [] });
        setFileToUpload(null); setUploadProgress(0);
    }, [caseData, isOpen]);
    ...
    const handleSubmit = async (e) => {
        ...
        await onSave(formData, fileToUpload, setUploadProgress);
        setLoading(false);
    };
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black bg-opacity-50 z-30 flex justify-center items-center p-4" onClick={onClose}>
            <motion.div initial={{ scale: 0.9, y: -20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: -20 }} className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b border-slate-200 flex justify-between items-center"><h2 className="text-xl font-bold text-slate-800">{caseData ? 'Edit Case File' : 'Add New Case File'}</h2><button onClick={onClose} className="p-1 rounded-full text-slate-500 hover:bg-slate-200"><X className="w-5 h-5" /></button></div>
                <form onSubmit={handleSubmit} className="overflow-y-auto p-6 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><InputField name="clientName" label="Client Name" value={formData.clientName} onChange={handleChange} required /><InputField name="caseTitle" label="Case Title" value={formData.caseTitle} onChange={handleChange} required /><InputField name="courtName" label="Court Name / Type" value={formData.courtName} onChange={handleChange} required /><InputField name="caseNumber" label="Case Number" value={formData.caseNumber} onChange={handleChange} required /><InputField name="opposingParty" label="Opposing Party" value={formData.opposingParty} onChange={handleChange} /><InputField name="fileLocation" label="Physical File Location" value={formData.fileLocation} onChange={handleChange} /></div>
                    ...
                    <div><label className="block text-sm font-medium text-slate-600 mb-1">Tags</label><div className="flex gap-2 mb-2"><input type="text" placeholder="Add a tag..." value={newTag} onChange={e => setNewTag(e.target.value)} className="flex-grow w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" /><button type="button" onClick={handleAddTag} className="bg-slate-200 text-slate-800 px-4 rounded-lg hover:bg-slate-300">Add</button></div><div className="flex flex-wrap gap-2">{formData.tags.map(tag => <span key={tag} className="flex items-center gap-2 bg-blue-100 text-blue-800 text-sm px-2 py-1 rounded-full">{tag}<button type="button" onClick={() => handleRemoveTag(tag)}><X className="w-3 h-3" /></button></span>)}</div></div>
